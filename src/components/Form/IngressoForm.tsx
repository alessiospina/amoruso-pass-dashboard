...
export default function IngressoForm({onSuccess, onError}: IngressoFormProps) {
    ...
    // Helper function per formattare la data per l'input datetime-local
    ...
    const getCurrentItalianDateTime = (): Date => {
        const now = new Date()
        // Crea un'istanza di Date in fuso orario italiano
        const italianTime = new Date(now.toLocaleString("en-US", {timeZone: "Europe/Rome"}))
        return italianTime
    }

    // Helper function per convertire il valore dell'input in Date
    ...
    const initialData: CreateIngressoDTO = {
        ...
        indirizzo: '',
        importo: 0,
        created_at: getCurrentItalianDateTime(),
    }
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'importo') {
            ...
            console.log(`📅 Data/ora selected: "${value}" -> ${dateValue}`)
            updateField('created_at' as keyof CreateIngressoDTO, dateValue)
        } else {
                ...
                <Card.Body className="p-3 p-md-4">
                    {error && (
                        <Alert variant="danger" dismissible onClose={() => setError(null)}
                               className="fw-bold text-white bg-danger border-danger">
                            {error}
                        </Alert>
                    )}
                            ...
                            <Col xs={12} lg={6}>
                                <Form.Group className="mb-3">
                                    ...
                                    <Form.Control
                                        type="datetime-local"
                                        name="created_at"
                                        value={formatDateForInput(formData.created_at || new Date())}
                                        onChange={handleInputChange}
                                        size="lg"
                                        required
                                        disabled={loading}
                                        isInvalid={shouldShowError('created_at')}
                                        className={shouldShowError('created_at') ? 'border-danger' : ''}
                                    />
                                    <Form.Control.Feedback type="invalid">
                                        {getErrorMessage('created_at')}
                                    </Form.Control.Feedback>
                                    <Form.Text className="text-muted">
                                        Data e ora dell'ingresso (si popola automaticamente con l'orario corrente)
                                    </Form.Text>
                                </Form.Group>
                            </Col>
                        </Row>
