...
export class IngressoService {
            ...
            if (data.created_at) {
                if (typeof data.created_at === 'string') {
                    ...
                    if (!isNaN(parsedDate.getTime())) {
                        normalizedData.created_at = parsedDate
                    }
                } else {
                    normalizedData.created_at = data.created_at
                }
            }
    ...
    async getIngressi(filters: IngressoFiltersDTO = {}, pagination: PaginationDTO = { page: 1, limit: 10 }): Promise<PaginatedResultDTO<Ingresso>> {
    ...
    async updateIngresso(id: string, data: UpdateIngressoDTO): Promise<Ingresso> {
        ...
        if (normalizedData.created_at) {
            if (typeof normalizedData.created_at === 'string') {
                const parsedDate = new Date(normalizedData.created_at)
                if (!isNaN(parsedDate.getTime())) {
                    normalizedData.created_at = parsedDate
                } else {
